...
app.get('/',async (request, response)=>{  //starts a get method when the root route is passed in, sets up req and res parameters
    const todoItems = await db.collection('plants').find().toArray() //setting a variable and awaits ALL items form the todos collection
    const itemsLeft = await db.collection('plants').countDocuments({completed: false}) //setting a variable and awaits a number of how many items that have a completed false - uncompleted - to display later in ejs
...
app.delete('/deleteItem', (request, response) => { //starts a delete method when the delete route is passed, req & res parameters
    db.collection('plants').deleteOne({thing: request.body.itemFromJS}) //using delete one method in our db collection - looking for the name from the span from ejs to main.js
